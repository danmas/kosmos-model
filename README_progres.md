...
## 2025-12-04: Валидация user_type моделей при старте сервера + UI

### Суть изменений

При запуске сервера теперь автоматически проверяются все модели с `user_type` (включая `CHEAP`, `FAST`, `RICH` и произвольные метки). Результаты отображаются:
1. **В логах** — с полным списком прошедших и не прошедших проверку
2. **В UI моделей** — кнопка "user_type" первой в ряду статистики, при ошибках красная с пульсацией

### Мотивация

Модели с `user_type` используются для маршрутизации запросов от внешних систем. Если такая модель недоступна (сервис упал, ключ истёк, модель deprecated), это критично. Раннее обнаружение проблем при старте позволяет:
- Быстро увидеть проблему в логах
- Не дожидаться первого неудачного запроса от клиента
- Сохранять результаты тестов в `last_test` для анализа
- Видеть статус валидации в интерфейсе управления моделями

### Изменения в файлах

**`server.js`**

1. **Глобальное хранилище `lastUserTypeValidation`**
   - Хранит результаты последней валидации: passed[], failed[], timestamp, inProgress

2. **Новая функция `testModelInternal(model)`**
   - Вынесена логика тестирования из `/api/test-model`
   - Возвращает объект `{ success, response_time_ms, sample_response, error_message, timestamp }`
   - Поддерживает все провайдеры: groq, openroute, direct, gigachat

3. **Обновлённая `validateUserTypeModelsOnStartup()`**
   - Сохраняет результаты в `lastUserTypeValidation`
   - Выводит итоговые списки прошедших и не прошедших проверку в лог

4. **Новые эндпоинты:**
   - `GET /api/user-type-validation` — получение результатов валидации
   - `POST /api/user-type-validation/rerun` — ручной перезапуск валидации

**`public/models.html`**
- Стили для кнопки валидации (зелёная/красная с пульсацией)
- Модальное окно с результатами валидации

**`public/models.js`**
- Загрузка данных валидации при инициализации
- Кнопка валидации первой в секции статистики
- Модальное окно с списками прошедших/не прошедших
- Возможность перезапуска валидации из UI

### UI

**Кнопка валидации:**
- Зелёная с текстом "N OK" если все модели прошли
- Красная с пульсацией и текстом "N ошибок" если есть проблемы
- При клике открывается модальное окно с деталями

**Модальное окно:**
- Секция "Не прошли проверку" (красная) — сверху, если есть ошибки
- Секция "Прошли проверку" (зелёная)
- Время последней проверки
- Кнопка "Перезапустить" для ручного запуска

### Пример лога при старте

```
🔍 Проверка моделей с user_type...
📋 Найдено 3 модел(ей) с user_type: CHEAP, FAST, RICH
🧪 Тестирование [CHEAP] → deepseek/deepseek-chat:free (openroute)...
✅ [CHEAP] OK (1234ms)
🧪 Тестирование [FAST] → llama-3.3-70b-versatile (groq)...
✅ [FAST] OK (567ms)
🧪 Тестирование [RICH] → gpt-4o (openroute)...
❌ [RICH] ОШИБКА: 403/401 — нет доступа (ключи/баланс)
   Модель: gpt-4o, Провайдер: openroute, ID: openroute-gpt-4o
...
❌ НЕ ПРОШЛИ ПРОВЕРКУ (1):
   • [RICH] gpt-4o (openroute) — 403/401 — нет доступа (ключи/баланс)
⚠️ ВНИМАНИЕ: Запросы с этими user_type могут завершаться ошибкой!
```
...
Изменена логика разрешения модели при использовании `user_type`. Теперь при указании `user_type` (включая `CHEAP`, `FAST`, `RICH` и произвольные метки) параметры `model` и `provider` из запроса **полностью игнорируются** — используются значения из конфигурации найденной модели.

### Мотивация

Внешние системы используют абстрактные метки (`FAST`, `MY_CUSTOM_TYPE`) для маршрутизации запросов, не зная конкретных моделей и провайдеров. Ранее переданный `provider` мог переопределить провайдера из конфигурации, что приводило к несогласованности.

### Приоритеты параметров

| Параметр | При `user_type` | Без `user_type` |
|----------|-----------------|-----------------|
| `model` | ❌ Из конфигурации | ✅ Из запроса |
| `provider` | ❌ Из конфигурации | ✅ Из запроса |
| `temperature` | ✅ Из запроса | ✅ Из запроса |
| `max_tokens` | ✅ Из запроса | ✅ Из запроса |

### Изменения в файлах

**`server.js`**
- Функция `resolveModelName()`: убран fallback `providerInput || modelByUserType.provider`
- Провайдер теперь всегда берётся из `modelByUserType.provider`
- Добавлен флаг `userTypeMatch: true` в результат для явной индикации

**Документация обновлена:**
- `README_REST.md` — новый раздел "Приоритет user_type"
- `swagger.yaml` — описание поведения в полях `model` и `provider`
- `README.md` — краткое упоминание в разделе "Умная маршрутизация"
...
{
  "model": "FAST",
  "provider": "openroute",  // ← игнорируется!
  "prompt": "...",
  "inputText": "..."
}

// Результат: используется модель с user_type="FAST" 
// и её провайдер (например, "groq"), а не "openroute"
```

---

## 2025-12-01: Автоматическая синхронизация моделей для Direct провайдера

### Суть изменений

Добавлена автоматическая синхронизация моделей для провайдера **Direct** (OpenAI-совместимые API). Теперь система может автоматически обновлять списки моделей от любых провайдеров, поддерживающих стандартный эндпоинт `/v1/models`.

### Как работает

1. **Группировка по base_url:** Система находит все модели с `provider: "direct"` и группирует их по уникальным `base_url`
2. **Запрос к каждому API:** Для каждого `base_url` запрашивается `{base_url}/v1/models`
3. **Тихая обработка ошибок:** Если эндпоинт не поддерживается (404), таймаут или ошибка авторизации — URL тихо пропускается без критических ошибок
4. **Обновление моделей:** Новые модели добавляются, устаревшие отключаются (`enabled: false`), ручные настройки сохраняются
...
- Формат `env:VAR_NAME` (например, `env:TOGETHER_API_KEY`)
- Прямые ключи
- Автоматический fallback на стандартные переменные окружения

**Обработка ошибок:**
- **404 Not Found** — `/v1/models` не поддерживается → пропускается
- **401/403** — ошибка авторизации → пропускается
- **Таймаут** (10 секунд) → пропускается
- Все ошибки обрабатываются тихо, без критических сообщений

### Изменения в файлах

**1. `server.js`**
- Новая функция `refreshDirectModels()` (строки ~2244-2380)
- Группировка моделей по `base_url`
- Запрос к `/v1/models` для каждого уникального URL
- Генерация уникальных ID на основе hostname и model_id
- Сохранение настроек существующих моделей
- Подключена к автоматической синхронизации (при старте и каждые 8 часов)

**2. `README_refreshModels.md`**
- Добавлен раздел о Direct провайдере с подробным описанием
- Примеры добавляемых моделей
- Описание логики синхронизации
- Информация об обработке ошибок

**3. `README.md`**
- Обновлен раздел "Автоматическая синхронизация моделей"
- Добавлена информация о поддержке Direct провайдера

### Поддерживаемые провайдеры синхронизации

| Провайдер | Статус | Особенности |
|-----------|--------|-------------|
| **Groq** | ✅ | Все модели, требует API ключ |
| **OpenRouter** | ✅ | Только бесплатные модели (`:free`) |
| **Direct** | ✅ | Все модели для каждого `base_url`, тихая обработка ошибок |
| **GigaChat** | ❌ | Ручное добавление |
...
```
→ Автоматически синхронизирует все модели от Together
...
```
→ Автоматически синхронизирует модели от Z.AI (если поддерживается `/v1/models`)
...
```
→ Автоматически синхронизирует локальные модели Ollama

### Логирование

При синхронизации выводится:
```
Обновляем список Direct моделей...
  Проверяем https://api.together.xyz/v1...
  ✅ https://api.together.xyz/v1: 5 новых, 0 отключено
  ⚠️ https://api.example.com: /v1/models не поддерживается, пропускаем
Direct синхронизирован: 5 новых добавлено, 0 устаревших отключено.
```
